...
int main(int argc, char* argv[])
{
    ...
    recvbuf = malloc(sizeof(double) * length);
    ...
    memset(sendbuf, 0, sizeof(double) * length);
    memset(recvbuf, 0, sizeof(double) * length);
    ...
    // start = MPI_Wtime();
    // if (rank == 0) {
    //     MPI_Send(sendbuf, length, MPI_DOUBLE, 1, 2025, MPI_COMM_WORLD);
    // }
